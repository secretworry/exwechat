defmodule WechatBase.Api.Endpoint.BodyType.Json.Schema do
  ...
  @type node_type :: primary_type | {:array, node_type} | :object
  ...
  @type compose_node :: {{:array, node_type}, id_t, node_opts, [schema_node]} | {:object, id_t, node_opts, [schema_node]}
  ...
  defp validate_children!(nil, _prefix) do
    []
  end

  defp validate_children!(children, prefix) when is_list(children) do
    Enum.map(children, &validate_schema_node!(&1, prefix))
  end

  defp validate_children!(illegal_children, prefix) do
  ...
  defp validate_schema_node!({node_type, identifier, node_opts}, prefix) when node_type in @primary_type do
    identifier = validate_identifier!(identifier, prefix)
    prefix = "#{prefix}.#{identifier}"
    {validate_node_type!(node_type, prefix), identifier, node_opts}
  end
  ...
  defp validate_schema_node!({node_type, identifier, node_opts, children}, prefix) do
    identifier = validate_identifier!(identifier, prefix)
    prefix = "#{prefix}.#{identifier}"
    {validate_node_type!(node_type, prefix), identifier, node_opts, validate_children!(children, prefix)}
  end
  ...
  defp validate_node_type!(node_type, prefix) when node_type in @primary_type or node_type == @object_type do
    node_type
  end

  defp validate_node_type!({:array, node_type}, prefix) do
    {:array, validate_node_type!(node_type, prefix)}
  end

  defp validate_node_type!(illegal_node_type, prefix) do
    raise ArgumentError, "Illegal node_type #{inspect illegal_node_type} at #{inspect prefix}"
  end

  defp validate_identifier!(identifier, prefix) when is_atom(identifier) or is_binary(identifier) do
    identifier
  end

  defp validate_identifier!(illegal_identifier, prefix) do
